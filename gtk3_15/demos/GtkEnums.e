...
------------------
namespace enums -- defines GTK constants and contains an index of widget names;
------------------
...
---------------------------------------------------------------------------------
-- ListView/TreeView storage types. Use these when creating new GtkListStores
-- or GtkTreeStores. They control how the column's data is displayed:
---------------------------------------------------------------------------------
public enum
  gBOOL = 20, -- displays values as TRUE or FALSE
  gINT = 24,  -- displays integer values (no d.p.)
  gFLT = 56,  -- displays floats with too much precision (6 d.p.), must be formatted
  gDBL = 60,  -- displays doubles with too much precision, should be formatted
  gSTR = 64   -- displays strings, as well as automatically formatting integers or atoms

public sequence storage_types = { -- for reverse lookup purposes;
  {20,"gBOOL"},
  {24,"gINT"},
  {56,"gFLT"},
  {60,"gDBL"},
...
public enum -- Response codes returned by button presses;
  MB_YES    = -8,
  MB_NO     = -9,
  MB_OK     = -5,
  MB_CANCEL = -6,
...
-----------------------------------------------------------------------------------------
-- a bazillion (more or less) GTK_CONSTANTS used to make code clearer. See the GTK docs.
-----------------------------------------------------------------------------------------
...
public enum type Cursor by 2 -- Cursors:
  GDK_X_CURSOR = 0,
  GDK_ARROW,
...
public enum type License
  ...
  GTK_LICENSE_AGPL_3_0, -- Affero GPL 3.22.27
  GTK_LICENSE_AGPL_3_0_ONLY -- Affero GPL 3.22.27
end type
...
-- arrows are deprecated
  ...
  GTK_INPUT_HINT_INHIBIT_OSK = 128, -- no on-screen keyboard
  GTK_INPUT_HINT_VERTICAL_WRITING = 256, -- 3.18
  GTK_INPUT_HINT_EMOJI = 512, -- 3.22
  GTK_INPUT_HINT_NO_EMOJI = 1024, -- 3.22
...
public enum -- event masks
  ...
  G_LOG_LEVEL_MASK = 256,

  G_PRIORITY_DEFAULT_IDLE = 200,
  G_PRIORITY_HIGH_IDLE = 100,
  G_PRIORITY_DEFAULT = 0,

  GDK_SEAT_CAPABILITY_NONE = 0, -- 3.20
...
global function log_err_name(integer x)
  return vlookup(x,log_err_names,1,2,"?")
end function
...
-- normally, they are only used when it is necessary to "type cast"
-- a parameter to a Eu function.
------------------------------------------------------------------------

global type Object(atom x) return _(x,GObject) end type
global type Window(object x) return _(x,GtkWindow) end type
